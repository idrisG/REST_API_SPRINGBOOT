...
class UserControllerIntegrationTest {
    ...
    //UserDTO user = new UserDTO("hubert_test","1996-01-01","France","0102030405",Gender.MALE.toString());
    //UserDTO user_wrong = new UserDTO("hubert_test!!!","1996-01-01","France","0102030405",Gender.MALE.toString());
	...
	public void testCreateUser() {
		...
		response = restTemplate.exchange(createUrlWithPort("/users"),
				HttpMethod.POST,entity, String.class);
		assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
	}
	/**
	 * Integration test method retrieves the user created while testing the create user method
	 * and assert that the response status is "302 Found".
	 * 
	 * Then tries to retrieve user using wrong username and assert that the responses status is "404 Not Found"
	 */
	@Test
	@Order(2)
	public void testRetireveUser() {
		HttpEntity<String> entity = new HttpEntity<>(null,headers);
		ResponseEntity<String> response = restTemplate.exchange(createUrlWithPort("/users/1"), 
				HttpMethod.GET, entity, String.class);
		assertEquals(HttpStatus.FOUND,response.getStatusCode());
		
		response = restTemplate.exchange(createUrlWithPort("/users/10"), 
				HttpMethod.GET, entity, String.class);
